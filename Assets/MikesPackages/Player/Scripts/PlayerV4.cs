...
public class PlayerV4 : MonoBehaviour
{
    ...
    [SerializeField] private float glidingGravityMultiplier = 0.2f;
    ...
    private void FixedUpdate()
    {
        ...
        // Tick down the gliding counter
        glidingCounter -= Time.deltaTime;
    ...
    public void activateGlide(InputAction.CallbackContext context)
    {
        if (context.performed && !isGrounded() && canGlide)
        {
            glidingCounter = glideDuration;
            rigidBody.velocity = new Vector2(rigidBody.velocity.x, 0.0f);
        }
    ...
    private void setGravityForState()
    {
        if (jumpCut)
        {
            print("jump cutting");
        ...
        {
            print("is gliding");
            setGravity(gravityScale * glidingGravityMultiplier);
        }
